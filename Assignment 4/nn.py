...
class Parameter(DataNode):
    ...
    def update(self, multiplier, direction):
...
class DotProduct(FunctionNode):
    ...
    Usage: nn.DotProduct(weights, features)
    Inputs:
    ...
    def _forward(*inputs):
        ...
        assert inputs[0].shape[0] == 1, (
            "First dimension of first input should be 1, instead got shape "
            "{}".format(format_shape(inputs[0].shape)))
        assert inputs[0].shape[1] == inputs[1].shape[1], (
            "Second dimension of inputs should match, instead got shapes {} "
            "and {}".format(
                format_shape(inputs[0].shape), format_shape(inputs[1].shape)))
        return np.dot(inputs[1], inputs[0].T)
